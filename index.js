...
//document.addEventListener(`DOMContentLoaded`, async function(event) {
  ...
  if (user) {
    ...
    let url = `/.netlify/functions/posts` // Note that this is a relative url, so that it will work in production and shared elsewhere
    ...
    for (let i = 0; i < json.length; i++) {
      ...
      // Add to comments markup
      let comments = post.comments
      for (let i=0; i<comments.length; i++) {
        let comment = comments[i]
        commentsMarkup = commentsMarkup + `<div><strong>${comment.userName}</strong> ${comment.body}</div>`
      }
